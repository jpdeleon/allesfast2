    ...
    for inst in config.BASEMENT.settings['inst_all']:
        if config.BASEMENT.settings['A_N_spots_'+inst] > 0:
            ...
            for i_sample, sample in tqdm(enumerate(posterior_samples)):
                ...
                baseline = allesfast.computer.calculate_baseline(params, inst, 'flux')

                model_xx = allesfast.computer.calculate_model(params, inst, 'flux', xx=xx) #evaluated on xx (!)
                baseline_xx = allesfast.computer.calculate_baseline(params, inst, 'flux', xx=xx) #evaluated on xx (!)
...
def plot_spots_from_posteriors(datadir, Nsamples=10, command='save'):
    ...
    config.init(datadir)
    posterior_samples_dic = allesfast.get_ns_posterior_samples(datadir, Nsamples=Nsamples)
