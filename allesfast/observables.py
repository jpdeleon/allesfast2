...
# The README comment above warns that astropy units cause a ~17x slow-down;
# these scalars let calc_rho / calc_rho_host run as plain float arithmetic.
...
_R_CM_SCALE: dict = {
    id(u.Rsun):  _RSUN_CM,
    id(u.Rjup):  _RJUP_CM,
    id(u.Rearth): _REARTH_CM,
}
_M_G_SCALE: dict = {
    id(u.Msun):  _MSUN_G,
    id(u.Mjup):  _MJUP_G,
    id(u.Mearth): _MEARTH_G,
}
...
def calc_rho_host(P, radius_1, rr, rho_comp,
    ...
    """
    if return_unit == 'cgs':
        # Fast path: plain float arithmetic.
        # P in days → seconds; rho_comp and result in g cm^-3.
        P_s = P * _DAY_S
