...
class Basement():
    ...
    def __init__(self, datadir, quiet=False):
        ...
        Returns:
        --------
        All the variables needed for allesfast
        '''
        ...
        #::: not repeated on every likelihood evaluation)
...
#        self.setup_GPs()
    ...
    def load_settings(self):
        '''
        For the full list of options see www.allesfast.com
        '''
        
        
        def set_bool(text):
            if text.lower() in ['true', '1','True']:
                return True
            else:
                return False
...
#        self.settings = {r[0]:r[1] for r in rows}
        ...
        #::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
        #::: Stellar-model priors
        #::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
        if 'use_torres_prior' in self.settings:
            self.settings['use_torres_prior'] = set_bool(self.settings['use_torres_prior'])
        else:
            self.settings['use_torres_prior'] = False

        if 'use_mist_prior' in self.settings:
            self.settings['use_mist_prior'] = set_bool(self.settings['use_mist_prior'])
        else:
            self.settings['use_mist_prior'] = False

        if 'use_sed_prior' in self.settings:
            self.settings['use_sed_prior'] = set_bool(self.settings['use_sed_prior'])
        else:
        ...
        if 'mcmc_burn_steps' not in self.settings:
            self.settings['mcmc_burn_steps'] = 0   # determined automatically at runtime
        ...
        if 'mcmc_ntemps' not in self.settings:
            self.settings['mcmc_ntemps'] = 1
        if 'mcmc_maxgr' not in self.settings:
        ...
        if self.settings['mcmc_burn_steps'] > 0 and \
           self.settings['mcmc_total_steps'] <= self.settings['mcmc_burn_steps']:
...
#                print('Using ns_sample=="slice".')
    ...
    ###############################################################################
    @staticmethod
    def _find_data_file(datadir, inst, ftype):
        """
