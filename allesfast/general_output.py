    ...
    #==========================================================================
        ...
        if (inst in base.settings['inst_rv']) or (inst in base.settings['inst_rv2']):
            ...
            if (len(x) > 500) or force_binning:
                ...
                #np.savetxt(os.path.join(config.BASEMENT.outdir, 'rv_phasefolded_data_'+inst+'.txt'), np.array([phi*zoomfactor, y, yerr_w]).T, fmt='%.7f')
            ...
            if (style in ['phase', 'phasezoom', 'phasezoom_occ', 'phase_curve']) and (samples is not None):
                #print(xx*zoomfactor, file=open(os.path.join(config.BASEMENT.outdir, 'rv_phasefolded_model_'+inst+'.txt'), 'w'))
...
def plot_initial_guess(return_figs=False, kwargs_dict=None):
    ...
    if return_figs==False:
        ...
        #::: stellar diagnostic plots at initial guess (if available)
